...
DEFAULT_DEV_JVM_ARGS="-Xms128m -Xmx384m -XX:MaxMetaspaceSize=160m -XX:MaxDirectMemorySize=128m -Xss512k -XX:+UseSerialGC -XX:+ExitOnOutOfMemoryError"
...
dev_jvm_args_for() {
  ...
  case "${name}" in
    mvflix-authorization) variable=MVFLIX_DEV_JVM_ARGS_AUTHORIZATION; service_defaults="${DEFAULT_DEV_JVM_ARGS/-Xmx384m/-Xmx256m}" ;;
    mvflix-users) variable=MVFLIX_DEV_JVM_ARGS_USERS; service_defaults="${DEFAULT_DEV_JVM_ARGS/-Xmx384m/-Xmx256m}" ;;
    mvflix-storage) variable=MVFLIX_DEV_JVM_ARGS_STORAGE; service_defaults="${DEFAULT_DEV_JVM_ARGS/-Xmx384m/-Xmx512m}" ;;
    mvflix-media-ingestion) variable=MVFLIX_DEV_JVM_ARGS_MEDIA_INGESTION; service_defaults="${DEFAULT_DEV_JVM_ARGS}" ;;
    bff-mvflix-web) variable=MVFLIX_DEV_JVM_ARGS_BFF; service_defaults="${DEFAULT_DEV_JVM_ARGS}" ;;
    *) variable="MVFLIX_DEV_JVM_ARGS_${name//-/_}"; service_defaults="${DEFAULT_DEV_JVM_ARGS}" ;;
  esac
  printf '%s\n' "${!variable:-${MVFLIX_DEV_JVM_ARGS:-${service_defaults}}}"
}
...
start_one() {
  ...
  local jvm_args
  jvm_args="$(dev_jvm_args_for "${name}")"
  local extra_args=()
  if [ "${name}" = "mvflix-media-ingestion" ]; then
    extra_args+=("-Dspring-boot.run.jvmArguments=${jvm_args} -DMEDIA_INGESTION_PORT_INTERNAL=${MEDIA_INGESTION_PORT} -DMVFLIX_INTERNAL_TOKEN_URI=http://127.0.0.1:${AUTH_PORT}/oauth2/token -DSECURITY_OAUTH2_JWK_SET_URI=http://127.0.0.1:${AUTH_PORT}/oauth2/jwks")
  elif [ "${name}" = "mvflix-playback" ]; then
    extra_args+=("-Dspring-boot.run.jvmArguments=${jvm_args} -DPLAYBACK_PORT_INTERNAL=${PLAYBACK_PORT} -DSERVICES_MOVIES_URL=http://127.0.0.1:${MOVIES_PORT} -DSERVICES_STORAGE_URL=http://127.0.0.1:${STORAGE_PORT} -DSERVICES_AUTHORIZATION_URL=http://127.0.0.1:${AUTH_PORT} -DSECURITY_OAUTH2_JWK_SET_URI=http://127.0.0.1:${AUTH_PORT}/oauth2/jwks -DKAFKA_BOOTSTRAP_SERVERS=${KAFKA_BOOTSTRAP_SERVERS:-127.0.0.1:9094}")
  elif [ "${name}" = "mvflix-activity" ]; then
    extra_args+=("-Dspring-boot.run.jvmArguments=${jvm_args} -DACTIVITY_PORT_INTERNAL=${ACTIVITY_PORT} -DSECURITY_OAUTH2_JWK_SET_URI=http://127.0.0.1:${AUTH_PORT}/oauth2/jwks -DKAFKA_BOOTSTRAP_SERVERS=${KAFKA_BOOTSTRAP_SERVERS:-127.0.0.1:9094} -DMVFLIX_MESSAGING_KAFKA_ENABLED=true")
  elif [ "${name}" = "bff-mvflix-web" ]; then
    extra_args+=("-Dspring-boot.run.jvmArguments=${jvm_args} -DMEDIA_INGESTION_URL=http://127.0.0.1:${MEDIA_INGESTION_PORT} -DPLAYBACK_URL=http://127.0.0.1:${PLAYBACK_PORT} -DACTIVITY_URL=http://127.0.0.1:${ACTIVITY_PORT}")
  elif [ "${name}" = "mvflix-users" ]; then
    extra_args+=("-Dspring-boot.run.jvmArguments=${jvm_args} -DDB_HOST=${DB_TARGET_HOST} -DDB_PORT=${DB_TARGET_PORT}")
  else
    extra_args+=("-Dspring-boot.run.jvmArguments=${jvm_args}")
  fi
